...
while running:
    ...
    for element in good_list:
        len(element)
    length_of_all_good = len(element) #defining the variable that will be used below, wanted to make sure this variable gets defined outside of the loop just in case. 

    #Input all the bad letters and put them in their own list for later use. 
    bad_list = [str(x).lower() for x in input("Enter any letters that you know are NOT in the list - (separate letters with a comma and space, also if you don't know any bad letters just hit space and then enter):  \n").split(', ')]
    ...
    if length_of_all_good <= 5:
            ...
            # this next line is basically saying look for all strings in the latter 5 (or less) user inputs, and search them against the 5 letter word list
                if all(x in string for x in good_list) and not any(x in string for x in bad_list): #compare good list to master list and compare bad list to master list. 
